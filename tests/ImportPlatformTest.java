...
public final class ImportPlatformTest {
  ...
  static void conflictsAndRollback(WorkflowPlatformTest.Fixture f)throws Exception {
    ...
    try(var db=WorkflowPlatformTest.connect(f.dir);var st=db.prepareStatement("SELECT payload FROM drafts WHERE id=?")){st.setString(1,draft.id());try(var rs=st.executeQuery()){check(rs.next()&&!WorkflowCodec.rows(rs.getString(1)).isEmpty()&&WorkflowCodec.rows(rs.getString(1)).get(0).change().values().containsValue("待审不丢失"),"visibility redaction does not rewrite persisted private draft payload");}}

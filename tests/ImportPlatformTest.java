...
public final class ImportPlatformTest {
  ...
  static void migration(WorkflowPlatformTest.Fixture f)throws Exception {
    ...
    f.open();check(f.store.schemaVersion()==7,"V3 migrates to V7");check(f.store.list(f.div,null,null,null).size()==records&&f.store.diagnostics().get("drafts")==drafts&&f.store.diagnostics().get("submissions")==submissions,"V3 official, drafts and submissions preserved");

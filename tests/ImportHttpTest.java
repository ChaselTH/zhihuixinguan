...
final class ImportHttpTest {
  ...
  static void run(HttpClient root,HttpClient division,HttpClient branch,HttpClient operator,HttpClient reviewer)throws Exception{
    ...
    var conflicting=uploadBundleFiles(List.of(october),List.of("2026-10-cross-conflict.xlsx"),csrf(),"2026-09");check(conflicting.statusCode()==400&&conflicting.body().contains("公共月份与文件名月份冲突"),"public month/file month conflict rejects whole upload");check(exportMatches(exportBytes("/export?dataset=cross&month=2026-10").body(),"cross-month-cross")==1,"conflicting upload leaves October official data unchanged");

...
final class ImportHttpTest {
  ...
  static void run(HttpClient root,HttpClient division,HttpClient branch,HttpClient operator,HttpClient reviewer)throws Exception{
    ...
    use(division);check(get("/foundation").body().contains("数据库结构：10"),"diagnostic page reflects actual migrated schema");
    ...
    String uploadPage=get("/imports").body();check(uploadPage.contains("action=\"/imports\"")&&uploadPage.contains("name=\"year\"")&&uploadPage.contains("name=\"monthNumber\"")&&!uploadPage.contains("name=\"period\""),"single upload route has required native month selectors and no business period field");
    byte[] september=bundleWorkbook("20260901-20260915","cross-month"),october=bundleWorkbook("20261001-20261015","cross-month");
    var crossMonth=uploadBundleFiles(List.of(september,october),List.of("2026-09-cross-month.xlsx","2026-10-cross-month.xlsx"),csrf(),"2026-10");
    check(crossMonth.statusCode()==200&&crossMonth.body().contains("冻结所属月份：2026-10")&&!crossMonth.body().contains("数据期次：2026-09"),"all files in one batch use the selected archive month");
    String crossToken=HttpSmokeTest.hidden(crossMonth.body()).get("token");var crossConfirm=post("/imports/confirm",Map.of("csrf",csrf(),"token",crossToken,"mode","saved","confirmSimilar","yes"));check(crossConfirm.statusCode()==303,"cross-month complete workbooks commit atomically; HTTP "+crossConfirm.statusCode()+" "+crossConfirm.body());
    ...
    check(!septemberDetails.contains("cross-month-cross"),"source dates do not archive rows into September");check(octoberDetails.contains("cross-month-cross"),"both distinct source rows archive under the selected month");
    check(exportMatches(exportBytes("/export?dataset=cross&month=2026-09").body(),"cross-month-cross")==0,"September export excludes selected-October rows");check(exportMatches(exportBytes("/export?dataset=cross&month=2026-10").body(),"cross-month-cross")==2,"October export includes both rows");
    var conflicting=uploadBundleFiles(List.of(october),List.of("2026-10-cross-conflict.xlsx"),csrf(),"2026-09");check(conflicting.statusCode()==200&&conflicting.body().contains("新增 3 条"),"same source uploaded in another chosen month becomes a separate period");check(exportMatches(exportBytes("/export?dataset=cross&month=2026-10").body(),"cross-month-cross")==2,"different-month preview leaves October data unchanged");
  ...
  static void sourceDates()throws Exception{
    Map<String,String> months=Map.of("multi","2031-01","negative","2031-01","cross","2031-01");
    ...
    check(response.statusCode()==200&&response.body().contains("新增 3 条"),"three sheets with different source dates stage independently of filename; HTTP "+response.statusCode()+" "+response.body());
    ...
    for(var schema:DatasetSchema.all()){
      String month=months.get(schema.id),key="RC8-DATES-"+schema.id,period=month;
      String selectedPage=get("/details?dataset="+schema.id+"&month="+month+"&q="+key).body();check(selectedPage.contains("虚构测试企业 "+key),"selected archive month controls monthly detail: "+schema.id);check(!selectedPage.contains("期次／历史保留信息"),"no extra metadata column in any detail");
      String yearly=get("/details?dataset="+schema.id+"&scope=year&year=2031&q="+key).body();check(yearly.contains("虚构测试企业 "+key),"year grouping follows the selected archive month");
      ...
      try(Workbook wb=WorkbookFactory.create(new ByteArrayInputStream(export.body()))){
        ...
        check(sh.getLastRowNum()==schema.headerRows&&row.getLastCellNum()==schema.width()&&row.getCell(schema.customerColumn).getStringCellValue().contains(key),"export has matching row and exact template width");check(row.getCell(schema.periodColumn<0?11:schema.periodColumn).getStringCellValue().equals(dates.get(schema.id)),"export preserves original source date");
      }
      ...
      var accepted=uploadBundleFiles(List.of(missing),List.of("20251101-20251115.xlsx"),csrf(),"2025-11");
      check(accepted.statusCode()==200&&accepted.body().contains("冻结所属月份：2025-11"),"missing source date is allowed with a chosen month");String missingToken=HttpSmokeTest.hidden(accepted.body()).get("token");check(post("/imports/confirm-bulk",Map.of("csrf",csrf(),"token",missingToken,"revision","1","mode","preserve","confirmed","yes")).statusCode()==303,"optional source date row commits in selected month");
      check(get("/details?dataset="+schema.id+"&month=2025-11&q=RC8-MISSING-"+schema.id).body().contains("虚构测试企业 RC8-MISSING"),"selected month archives a row with an empty source date");
    }
    var again=uploadBundleFiles(List.of(file),List.of("20391201-20391215.xlsx"),csrf(),"2031-01");check(again.statusCode()==200&&again.body().contains("重复 3 条"),"same source and selected month identifies all rows as duplicates");
    ...
    var otherMonth=uploadBundleFiles(List.of(file),List.of("20391201-20391215.xlsx"),csrf(),"2032-01");check(otherMonth.statusCode()==200&&otherMonth.body().contains("新增 3 条"),"same source values in a different selected month make independent records");String otherToken=HttpSmokeTest.hidden(otherMonth.body()).get("token");check(post("/imports/cancel",Map.of("csrf",csrf(),"token",otherToken,"revision","1")).statusCode()==303,"release other-month preview");
  }
  ...
  static HttpResponse<String> uploadDataset(String dataset,List<byte[]> files,List<String> names,String csrf)throws Exception{
    ...
    out.write(("--"+boundary+"--\r\n").getBytes(StandardCharsets.UTF_8));return HttpSmokeTest.client.send(HttpRequest.newBuilder(URI.create(HttpSmokeTest.base+"/imports")).timeout(java.time.Duration.ofSeconds(30)).header("Content-Type","multipart/form-data; boundary="+boundary).POST(HttpRequest.BodyPublishers.ofByteArray(out.toByteArray())).build(),HttpResponse.BodyHandlers.ofString());
  }

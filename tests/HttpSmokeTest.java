...
public final class HttpSmokeTest {
  ...
  public static void main(String[] args)throws Exception{
    ...
    try{
      ...
      String branchEdit=get("/details?dataset=multi&month=2026-09").body();fields=hidden(branchEdit);String rowId=fields.get("id0");for(int c=0;c<schema.width();c++)if(schema.editable(c))fields.put("v0_"+c,c==17?"HTTP 虚构反馈 <script>test</script>":"");
      var editPreview=post("/update-batch",fields);check(editPreview.statusCode()==200,"branch edit previews before division final review");
      check(!get("/details?dataset=multi&month=2026-09").body().contains("HTTP 虚构反馈"),"legacy editor cannot bypass confirmation");
      var confirmation=hidden(editPreview.body());var branchSubmit=post("/workflow/confirm",confirmation);check(branchSubmit.statusCode()==200,"branch confirmation creates a division task");
      var repeatedConfirm=post("/workflow/confirm",confirmation);check(repeatedConfirm.statusCode()==200,"same confirmation request is idempotent: "+repeatedConfirm.statusCode()+" "+repeatedConfirm.body().substring(0,Math.min(180,repeatedConfirm.body().length())));
      client=divisionClient;String submitId=WorkflowIntegrationHttpTest.id(branchSubmit.body());check(post("/workflow/review/approve",WorkflowIntegrationHttpTest.form(get("/workflow/submission?id="+submitId).body(),"/workflow/review/approve")).statusCode()==200,"division approves branch change");
      ...
      var stalePreview=upload("multi",multi,csrf);Map<String,String> reopen=WorkflowIntegrationHttpTest.form(get("/workflow/reopen?record="+rowId).body(),"/workflow/reopen");reopen.put("reason","请支行重新核对");reopen.put("confirm","yes");check(post("/workflow/reopen",reopen).statusCode()==303,"division reopens completed row for branch correction");
      client=branchClient;Map<String,String> next=hidden(get("/details?dataset=multi&month=2026-09").body());for(int c=0;c<schema.width();c++)if(schema.editable(c))next.put("v0_"+c,c==17?"另一次虚构保存":"");
      var newer=post("/workflow/confirm",hidden(post("/update-batch",next).body()));check(newer.statusCode()==200,"branch correction waits for division rather than publishing");
      ...
      client=branchClient;fields.put("requestId",UUID.randomUUID().toString());check(post("/update-batch",fields).statusCode()==409,"stale branch editor rejected");client=divisionClient;

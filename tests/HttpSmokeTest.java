...
public final class HttpSmokeTest {
  ...
  public static void main(String[] args)throws Exception{
    ...
    try{
      ...
      check(get("/health").body().contains("SCHEMA=10"),"health reports migrated schema version");
      ...
      var detected=upload("negative",multi,csrf);check(detected.statusCode()==200&&detected.body().contains("本批数据对比"),"single upload entrance detects workbook schema, not caller-supplied type");check(post("/imports/cancel",Map.of("csrf",csrf,"token",hidden(detected.body()).get("token"))).statusCode()==303,"release auto-detection preview before subsequent fixtures");
      ...
      check(!details.contains("保存资料补充")&&!details.contains("<textarea")&&!details.contains("支行修改记录"),"division list is read-only without page-level edit/history controls");
      ...
      check(post("/update-batch",fields).statusCode()==403,"division cannot forge the old edit form");
      client=newClient();loginAndChange("900000003",branch.password());HttpClient branchClient=client;
      String branchEdit=get("/details?dataset=multi&month=2026-09").body();check(!branchEdit.contains("/update-batch")&&!branchEdit.contains("<textarea"),"branch manager can supervise but not edit");
      check(post("/update-batch",fields).statusCode()==403,"obsolete direct edit cannot be forged by branch manager");
      client=newClient();loginAndChange("900000004",operator.password());HttpClient operatorClient=client;
      String opEdit=get("/details?dataset=multi&month=2026-09").body();var operatorForm=WorkflowIntegrationHttpTest.form(opEdit,"/workflow/draft/save");String rowId=operatorForm.get("id0");operatorForm.put("value_0_feedback","HTTP 虚构反馈 <script>test</script>");operatorForm.put("intent","preview");
      var editPreview=post("/workflow/draft/save",operatorForm);check(editPreview.statusCode()==200,"operator sees server confirmation before submitting");
      var confirmation=WorkflowIntegrationHttpTest.form(editPreview.body(),"/workflow/confirm");var operatorSubmit=post("/workflow/confirm",confirmation);check(operatorSubmit.statusCode()==200,"operator submission creates branch review");
      var repeatedConfirm=post("/workflow/confirm",confirmation);check(repeatedConfirm.statusCode()==200,"same confirmation request is idempotent");
      String submitId=WorkflowIntegrationHttpTest.id(operatorSubmit.body());check(!get("/details?dataset=multi&month=2026-09").body().contains("虚构测试企业 HTTP"),"operator row disappears during branch review");
      client=newClient();loginAndChange("900000005",reviewer.password());HttpClient reviewerClient=client;
      check(post("/workflow/review/approve",WorkflowIntegrationHttpTest.form(get("/workflow/submission?id="+submitId).body(),"/workflow/review/approve")).statusCode()==200,"reviewer advances row to division");
      client=divisionClient;check(post("/workflow/review/approve",WorkflowIntegrationHttpTest.form(get("/workflow/submission?id="+submitId).body(),"/workflow/review/approve")).statusCode()==200,"division final review publishes operator change");
      ...
      var stalePreview=upload("multi",multi,csrf);Map<String,String> reopen=WorkflowIntegrationHttpTest.form(get("/workflow/reopen?record="+rowId).body(),"/workflow/reopen");reopen.put("reason","请支行重新核对");reopen.put("confirm","yes");var reopened=post("/workflow/reopen",reopen);check(reopened.statusCode()==303,"division reopens completed row to reviewer");
      String reopenedId=WorkflowIntegrationHttpTest.match(reopened.headers().firstValue("location").orElse(""),"id=([^&]+)");
      client=operatorClient;check(!get("/details?dataset=multi&month=2026-09").body().contains("虚构测试企业 HTTP"),"operator cannot see reopened row while reviewer owns it");
      client=reviewerClient;String revisionPage=get("/workflow/submission?id="+reopenedId).body();String revisionUrl=WorkflowIntegrationHttpTest.unescape(WorkflowIntegrationHttpTest.match(revisionPage,"href=\"([^\"]+)\">自行修改后送分行"));Map<String,String> revision=WorkflowIntegrationHttpTest.form(get(revisionUrl).body(),"/workflow/review/revise");revision.put("value_feedback","另一次虚构保存");revision.put("intent","save");var revisionSaved=post("/workflow/review/revise",revision);check(revisionSaved.statusCode()==303,"reviewer saves private correction");revision=WorkflowIntegrationHttpTest.form(get(revisionSaved.headers().firstValue("location").orElseThrow()).body(),"/workflow/review/revise");revision.put("intent","submit");var newer=post("/workflow/review/revise",revision);check(newer.statusCode()==200,"reviewer submits correction to division");
      ...
      check(post("/workflow/review/approve",WorkflowIntegrationHttpTest.form(get("/workflow/submission?id="+WorkflowIntegrationHttpTest.id(newer.body())).body(),"/workflow/review/approve")).statusCode()==200,"division publishes corrected row");
      client=branchClient;fields.put("requestId",UUID.randomUUID().toString());check(post("/update-batch",fields).statusCode()==403,"obsolete branch edit remains closed");client=divisionClient;
      ...
      var pendingBranch=upload("multi",workbook("multi","WUJIN","BRANCH-PENDING"),csrf);check(post("/imports/confirm",Map.of("csrf",csrf,"token",hidden(pendingBranch.body()).get("token"),"mode","preserve")).statusCode()==303,"unfinished row remains in branch work queue");
      ...
      String branchDetails=get("/details?dataset=multi&month=2026-09").body();check(branchDetails.contains("虚构测试企业 BRANCH-PENDING")&&!branchDetails.contains("虚构测试企业 HTTP")&&!branchDetails.contains("虚构测试企业 OTHER"),"branch work queue includes its pending row and hides completed/foreign rows");
      ...
      check(!get("/details?dataset=multi&month=2026-09").body().contains("保存资料补充"),"reviewer cannot start initial filling directly");
      ...
      Feedback009HttpReviewTest.run(superClient,divisionClient,branchClient,operatorClient,reviewerClient);
      client=superClient;

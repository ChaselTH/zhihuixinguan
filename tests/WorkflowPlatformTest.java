...
public final class WorkflowPlatformTest {
  ...
  static void migrations()throws Exception {
    ...
    try(PlatformStore store=new PlatformStore(dir)){
      check(store.schemaVersion()==6,"V1 upgrades to V6");var oldUser=store.authenticateUser("000000001",password);check(oldUser!=null,"V1 credential preserved");
    ...
    try(Connection db=connect(dir);Statement st=db.createStatement();ResultSet rs=st.executeQuery("SELECT version,checksum FROM schema_migrations ORDER BY version")) {
      check(rs.next()&&rs.getInt(1)==1&&rs.getString(2).equals(Codec.hash(v1)),"original V1 checksum unchanged");check(rs.next()&&rs.getInt(1)==2&&rs.next()&&rs.getInt(1)==3&&rs.next()&&rs.getInt(1)==4&&rs.next()&&rs.getInt(1)==5&&rs.next()&&rs.getInt(1)==6&&!rs.next(),"six sequential schema versions");
    }
    ...
    try(Connection db=connect(dir);Statement st=db.createStatement()){st.execute("INSERT INTO schema_migrations VALUES(7,'future','2026-09-14T00:00:00Z')");}

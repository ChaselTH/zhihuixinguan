...
final class BusinessHttpTest {
  ...
  static void run(HttpClient root,HttpClient division,HttpClient branch,HttpClient operator,HttpClient reviewer)throws Exception{
    ...
    use(reviewer);var pageDecision=WorkflowIntegrationHttpTest.formLast(get("/workflow/submission?id="+unifiedId).body(),"/workflow/review/approve");check(post("/workflow/review/approve",pageDecision).statusCode()==200,"cross-page snapshot advances after branch review");
    use(division);check(post("/workflow/review/approve",WorkflowIntegrationHttpTest.formLast(get("/workflow/submission?id="+unifiedId).body(),"/workflow/review/approve")).statusCode()==200,"cross-page snapshot publishes after division review");
    use(operator);String submissionHistory=get("/records/history?organization=WUJIN&dataset=multi&search="+java.net.URLEncoder.encode(unifiedId,java.nio.charset.StandardCharsets.UTF_8)).body();check(submissionHistory.contains("B2-CROSS-PAGE-ONE")&&submissionHistory.contains("B2-CROSS-PAGE-TWO"),"history shows both official field changes for exact submission: "+submissionHistory.substring(Math.max(0,submissionHistory.length()-1200)));
    ...
    use(division);check(post("/workflow/review/approve",WorkflowIntegrationHttpTest.formLast(get("/workflow/submission?id="+submissionId).body(),"/workflow/review/approve")).statusCode()==200,"division final review publishes official data");
    use(operator);check(!get(url).body().contains("虚构测试企业 B2-HTTP-FOCUS"),"incomplete branch queue removes finally completed row");
    String completed=get(url.replace("completion=incomplete","completion=complete")).body();check(!completed.contains("虚构测试企业 B2-HTTP-FOCUS"),"branch cannot enumerate completed rows through completion filter");
    check(exportCount("incomplete")==0&&exportCount("complete")==0,"branch XLSX matches its pending-only visibility scope");check(!get("/?month=2026-09").body().contains("B2-HTTP-FOCUS"),"homepage shows only visible pending rows");
    use(division);String divisionCompleted=get(url.replace("completion=incomplete","completion=complete")).body();check(divisionCompleted.contains("虚构测试企业 B2-HTTP-FOCUS")&&divisionCompleted.contains("row-complete")&&divisionCompleted.contains("B2-HTTP-PRIVATE &lt;script&gt;text&lt;/script&gt;"),"division retains full completed values and presentation");
    ...
    use(division);String home=get("/?month=2026-09").body();check(home.contains("交叉违约清单 · 待处理")&&home.contains("查看全部待处理")&&home.contains("href=\"/progress?")&&home.contains("branch-progress"),"three pending lists and linked progress on homepage");
    ...
    use(branch);check(get("/workflow/reopen?record="+ownId).statusCode()==403&&get(focus).statusCode()==403,"branch roles cannot reopen or address completed rows directly");
    use(division);String reopenPage=get("/workflow/reopen?record="+ownId).body();var reopen=WorkflowIntegrationHttpTest.form(reopenPage,"/workflow/reopen");reopen.put("confirm","yes");check(post("/workflow/reopen",reopen).statusCode()==400,"completed row reopen requires a reason");reopen.put("reason","终审后发现需补充核验");check(post("/workflow/reopen",reopen).statusCode()==303,"division reopens exactly one completed row with a reason");
    use(branch);String reopenedDetails=get("/details?dataset=multi&month=2026-09&q=B2-HTTP-FOCUS&completion=incomplete").body();check(reopenedDetails.contains("退回待修改")&&reopenedDetails.contains("终审后发现需补充核验")&&reopenedDetails.contains("B2-HTTP-PRIVATE"),"reopened row preserves official value and shows reason to branch");
    String direct=get(focus).body();form=WorkflowIntegrationHttpTest.form(direct,"/workflow/direct/preview");form.put("value_0_feedback","B2-HTTP-REVISED");preview=post("/workflow/direct/preview",form);var reopenedSubmit=post("/workflow/confirm",HttpSmokeTest.hidden(preview.body()));check(reopenedSubmit.statusCode()==200&&reopenedSubmit.body().contains("待分行终审"),"branch update after reopen starts a new division-review chain");
    use(division);String reopenSubmission=WorkflowIntegrationHttpTest.id(reopenedSubmit.body());check(post("/workflow/review/approve",WorkflowIntegrationHttpTest.form(get("/workflow/submission?id="+reopenSubmission).body(),"/workflow/review/approve")).statusCode()==200,"division republishes revised row in new chain");

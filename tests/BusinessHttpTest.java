...
final class BusinessHttpTest {
  ...
  static void run(HttpClient root,HttpClient division,HttpClient branch,HttpClient operator,HttpClient reviewer)throws Exception{
    ...
    var unifiedConfirmed=post("/workflow/confirm",WorkflowIntegrationHttpTest.form(unifiedPreview.body(),"/workflow/confirm"));String today=java.time.LocalDate.now(java.time.ZoneId.of("Asia/Shanghai")).toString();check(unifiedConfirmed.statusCode()==200,"unified cross-page submission confirms");check(get("/records/history?organization=WUJIN&dataset=multi&from="+today+"&through="+today).body().contains("B2-CROSS-PAGE-ONE"),"history date filters submission event time rather than source period");

...
final class BusinessHttpTest {
  ...
  static void run(HttpClient root,HttpClient division,HttpClient branch,HttpClient operator,HttpClient reviewer)throws Exception{
    ...
    for(var client:List.of(root,division,branch,operator,reviewer)){use(client);var internal=get("/internal?scope=year&year=2026&completion=incomplete");check(internal.statusCode()==200&&internal.body().contains("待配置")&&internal.body().contains("dataset=cross"),"internal configured data and unknown-template empty state for all roles");}

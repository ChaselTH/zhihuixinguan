...
public final class Feedback009ReviewTest {
  ...
  public static void main(String[] args)throws Exception {
    ...
    run("R6 historical publication returns to reviewer",Feedback009ReviewTest::reconfirmation);
    run("R6b historical reviewer revision survives another division return",Feedback009ReviewTest::historicalReviewerRoundTrip);
  ...
  static void deletion()throws Exception {try(var f=new Fixture()){
    ...
    var s=f.pending(f.op,a,"delete-protected");f.w().approve(f.review,s.id(),id());
  ...
  static void singleAdministrator()throws Exception {try(var f=new Fixture()){
    ...
    var own=f.record("WUJIN","multi");code(Code.VERSION_CONFLICT,()->publish(f.store,f.review,edit(own,"reviewer-authored")));
  }}
  ...
  static void reconfirmation()throws Exception {try(var f=new Fixture()){
    ...
    long notices=f.n().unreadCount(f.review);String repeat=id();String task=f.w().reopenCompleted(f.div,row.id(),row.version(),"please verify",repeat);
    verify(f.w().reopenCompleted(f.div,row.id(),row.version(),"please verify",repeat).equals(task)&&f.n().unreadCount(f.review)==notices+1,"same reopen request remains exactly once and alerts reviewer");
    verify(f.store.find(f.div,row.id()).workflowStage()==RowStage.BRANCH_REVIEW&&value(f.store.find(f.div,row.id())).equals("unchanged and correct"),"formal value retained while reviewer handles reopened task");
    verify(!f.w().mayReject(f.review,task),"historical publication without original operator cannot be returned to a random operator");
    String recordId=row.id();code(Code.ALREADY_SUBMITTED,()->f.w().saveDraft(f.op,"",0,"multi",List.of(edit(f.store.find(f.op,recordId),"forbidden")),"",id()));
    f.w().approve(f.review,task,id());f.w().approve(f.div,task,id());
    verify(f.store.find(f.div,row.id()).workflowStage()==RowStage.PUBLISHED&&value(f.store.find(f.div,row.id())).equals("unchanged and correct"),"reviewer and division may confirm a historical row without changing formal content");
  }}
  static void historicalReviewerRoundTrip()throws Exception {try(var f=new Fixture()){
    var row=f.record("WUJIN","multi");publish(f.store,f.div,edit(row,"historical value"));
    row=f.store.find(f.div,row.id());String reopened=f.w().reopenCompleted(f.div,row.id(),row.version(),"请复核并修订",id());
    Map<String,String> proposed=new LinkedHashMap<>();var schema=DatasetSchema.get("multi");
    for(var field:schema.fields)if(field.editable())proposed.put(field.key(),schema.value(row.values(),schema.index(field.key())));
    proposed.put("feedback","reviewer correction");
    var revised=f.w().reviseForDivision(f.review,reopened,row.id(),row.version(),proposed,id());
    verify(f.store.find(f.div,row.id()).workflowStage()==RowStage.DIVISION_REVIEW,"reviewer correction waits for division without publishing");
    f.w().reject(f.div,revised.id(),"还需复核",id());
    verify(f.store.find(f.div,row.id()).workflowStage()==RowStage.BRANCH_REVIEW&&!f.w().mayReject(f.review,revised.id()),"second return stays with reviewer when historical author is unknown");
    f.w().approve(f.review,revised.id(),id());f.w().approve(f.div,revised.id(),id());
    verify(value(f.store.find(f.div,row.id())).equals("reviewer correction"),"reviewer correction becomes formal only after second branch and division approvals");
  }}
  static void legacyReopen()throws Exception {try(var f=new Fixture()){
    ...
    verify(f.store.find(f.op,row.id()).workflowStage()==RowStage.BRANCH_REVIEW&&value(f.store.find(f.op,row.id())).equals("legacy-complete"),"legacy formal values survive reviewer-first reopening");
  }}
  static void partialReturn()throws Exception {try(var f=new Fixture()){
    ...
    denied(()->f.w().saveDraft(f.op2,"",0,"negative",List.of(edit(f.store.find(f.op2,b.id()),"SUCCESSOR-B")),s.id(),id()));
  ...
  static void transactions()throws Exception {try(var f=new Fixture()){
    ...
    String reopened=f.w().reopenCompleted(f.div,a.id(),2,"published reviewer correction",id());
    verify(f.store.find(f.div,a.id()).workflowStage()==RowStage.BRANCH_REVIEW&&f.w().submission(f.review,reopened).rowStages().get(a.id())==RowStage.BRANCH_REVIEW,"completed row reopens to reviewer without restoring the original private draft");
    code(Code.ALREADY_DECIDED,()->f.w().editableDraft(f.op,d.id()));
    f.w().approve(f.review,reopened,id());f.w().approve(f.div,reopened,id());
    verify(f.store.find(f.div,a.id()).workflowStage()==RowStage.PUBLISHED,"reviewer and division can close a no-change recheck atomically");
  }}

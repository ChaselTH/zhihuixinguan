...
final class CompletionRuleHttpTest {
  ...
  static void run(HttpClient root,HttpClient division,HttpClient branch,HttpClient operator,HttpClient reviewer)throws Exception {
    ...
    for(var schema:DatasetSchema.all()){
      ...
      var fill=form(get(detail).body(),"/workflow/draft/save");fill.put("value_0_"+choice,"否");fill.put("intent","preview");preview=post("/workflow/draft/save",fill);check(preview.statusCode()==200,"partial formal row supports a new operator preview: "+preview.statusCode()+" "+preview.body().substring(Math.max(0,preview.body().indexOf("<main")),Math.min(preview.body().length(),Math.max(0,preview.body().indexOf("<main"))+900)));var firstFinal=post("/workflow/confirm",form(preview.body(),"/workflow/confirm"));check(firstFinal.statusCode()==200&&!get(detail).body().contains("✓ 正式已完成"),"operator may complete remaining required field through ordinary review chain");
      String firstFinalId=WorkflowIntegrationHttpTest.id(firstFinal.body());use(reviewer);check(post("/workflow/review/approve",form(get("/workflow/submission?id="+firstFinalId).body(),"/workflow/review/approve")).statusCode()==200,"reviewer approves remaining required value");
      use(division);check(post("/workflow/review/approve",form(get("/workflow/submission?id="+firstFinalId).body(),"/workflow/review/approve")).statusCode()==200,"division final approval publishes required value");
      ...
      String rowId=BusinessHttpTest.recordId(get("/details?dataset="+schema.id+"&month=2026-09&q="+key+"&completion=complete").body());var reopen=form(get("/workflow/reopen?record="+rowId).body(),"/workflow/reopen");reopen.put("confirm","yes");reopen.put("reason","规则回归：终审后重新打开");var reopened=post("/workflow/reopen",reopen);check(reopened.statusCode()==303,"division can reopen a completed row with reason");String reopenedId=WorkflowIntegrationHttpTest.match(reopened.headers().firstValue("location").orElseThrow(),"id=([^&]+)");
      use(reviewer);String reviewHtml=get("/workflow/submission?id="+reopenedId).body();String editUrl=WorkflowIntegrationHttpTest.unescape(WorkflowIntegrationHttpTest.match(reviewHtml,"href=\"([^\"]+)\">自行修改后送分行"));fill=form(get(editUrl).body(),"/workflow/review/revise");fill.put("value_"+choice,"");fill.put("intent","submit");var clearPending=post("/workflow/review/revise",fill);check(clearPending.statusCode()==200,"reopened completion change waits for a new division review");

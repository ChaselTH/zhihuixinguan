...
final class CompletionRuleHttpTest {
  ...
  static void run(HttpClient root,HttpClient division,HttpClient branch,HttpClient operator,HttpClient reviewer)throws Exception {
    ...
    for(var schema:DatasetSchema.all()){
      ...
      String rowId=HttpSmokeTest.hidden(get("/details?dataset="+schema.id+"&month=2026-09&q="+key+"&completion=complete").body()).get("id0");var reopen=form(get("/workflow/reopen?record="+rowId).body(),"/workflow/reopen");reopen.put("confirm","yes");reopen.put("reason","规则回归：终审后重新打开");check(post("/workflow/reopen",reopen).statusCode()==303,"division can reopen a completed row with reason");

...
final class AccessHttpTest {
  ...
  static void run(HttpClient root,HttpClient division,HttpClient branch,HttpClient operator)throws Exception {
    ...
    String notices=get("/notifications").body();String notice=match(notices,"/notifications/detail\\?id=([^\"]+)");
    ...
    use(root);Map<String,String> read=HttpSmokeTest.hidden(get("/notifications/detail?id="+notice).body());check(post("/notifications/read",read).statusCode()==303,"personal mark read");
    ...
    use(HttpSmokeTest.newClient());apply("870000001","重复已有账号","WUJIN");apply("870000004","虚构限流前","WUJIN");
    Map<String,String> limited=HttpSmokeTest.hidden(get("/access/apply").body());limited.put("number","870000005");limited.put("name","虚构限流");limited.put("organization","WUJIN");

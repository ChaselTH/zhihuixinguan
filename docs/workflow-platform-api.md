...
2026-09-21 合并前修复说明：FEEDBACK-009 的两级审批、行级回执、导入候选、退回恢复和迁移契约见 [FEEDBACK-009 当前接口补充](FEEDBACK-009-workflow-contracts.md)。下文保留为 PR0.7 历史交接，状态枚举、直接修改和整单退回限制不再代表当前接口。
...
A1 补充说明：工作流公共接口沿用本文件签名；权限、个人通知详情与公共审计查询的新增 API 见 [A1 交接](A1-权限与公共页面交接.md)。B1 不需要复制或修改上述 A1 公共实现。

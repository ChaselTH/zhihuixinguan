...
final class NotificationPages extends PageLayout {
  ...
  String detail(NotificationService.Notice n,String application){return page("通知详情",header()+"<div class=\"page-shell\"><section class=\"identity-card\"><h1>"+e(n.title())+"</h1><p>"+e(time(n.createdAt().toString()))+" · "+e(Organizations.label(n.organizationId()))+"</p><p class=\"access-pre\">"+e(n.summary())+"</p>"+(!application.isEmpty()?"<p><a class=\"btn btn-primary\" href=\"/access/request?id="+u(application)+"\">查看权限申请</a></p>":!n.submissionId().isEmpty()?"<p>工作流单据编号：<code>"+e(n.submissionId())+"</code></p><p class=\"field-note\">工作流审批页面将在 B1 合入后接入；通知已保存。</p>":"")+"<form method=\"post\" action=\"/notifications/read\">"+hidden("csrf",currentSession.csrf)+hidden("id",n.id())+"<button class=\"btn btn-light\" type=\"submit\">标记已读并返回</button></form></section></div>");}
}

...
final class FeedbackPages extends PageLayout {
  ...
  String home(DashboardData data){
    ...
    StringBuilder b=new StringBuilder("<section class=\"panel feedback-panel\"><div class=\"panel-head\"><h2>本期反馈时间</h2><a class=\"btn btn-light\" href=\"/deadlines?").append(e(data.range.queryString())).append("&amp;return=").append(u("/?"+data.range.queryString())).append("\">").append(manager?"设置反馈截止日期":"查看全部期次").append("</a></div>");
  ...
  String settings(DashboardData data,String notice){
    return settings(data,notice,"/?"+data.range.queryString());
  }
  String settings(DashboardData data,String notice,String source){
    boolean manager=AccessPolicy.all(currentSession.actor);
    String back=ReturnNavigation.safe(source,"/?"+data.range.queryString());
    StringBuilder b=new StringBuilder(backButton(back)).append("<h1>反馈截止日期</h1>");
    if(!notice.isEmpty())b.append("<p class=\"alert alert-success\">").append(e(notice)).append("</p>");
    b.append(rangeForm("/deadlines",data,hidden("return",back))).append("<p>按清单和来源期次分别设置，对全部支行生效。新设或修改只能选择北京时间今天之后的日期（最早明天）。截止当天结束前有效；已有日期到期后仍保留，超期不阻止继续填写和复核。</p>");
  ...
  private String link(DashboardData data,DashboardData.FeedbackPeriod p,String completion,String source){
    return ReturnNavigation.link("/details?"+new BusinessFilter(p.dataset,"","",completion,20,p.period).query(data.range),source);
  }
}

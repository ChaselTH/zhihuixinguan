...
final class BusinessFilter {
  final String dataset,branch,search,completion,period,draftId;final int pageSize;
  ...
  BusinessFilter(String dataset,String branch,String search,String completion,int pageSize,String period,String draftId){this.period=period;this.draftId=draftId==null?"":draftId.strip();this.pageSize=pageSize;this.dataset=dataset;this.branch=branch;this.search=search;this.completion=completion;}
  static BusinessFilter from(ActorContext actor,Map<String,String> query){
    ...
    return new BusinessFilter(dataset,branch,HttpSupport.limit(query.get("q"),100),completion(query.get("completion")),pageSize(query.get("pageSize")),HttpSupport.limit(query.get("period"),100),HttpSupport.limit(query.get("draft"),120));
  }
  ...
  String query(RangeSelection range){StringBuilder q=new StringBuilder(range.queryString()).append("&dataset=").append(PageLayout.u(dataset)).append("&branch=").append(PageLayout.u(branch)).append("&q=").append(PageLayout.u(search)).append("&period=").append(PageLayout.u(period)).append("&pageSize=").append(pageSize).append("&completion=").append(completion);if(!draftId.isBlank())q.append("&draft=").append(PageLayout.u(draftId));return q.toString();}
  String hidden(){StringBuilder b=new StringBuilder(PageLayout.hidden("dataset",dataset)).append(PageLayout.hidden("branch",branch)).append(PageLayout.hidden("q",search)).append(PageLayout.hidden("completion",completion)).append(PageLayout.hidden("pageSize",""+pageSize)).append(PageLayout.hidden("period",period));if(!draftId.isBlank())b.append(PageLayout.hidden("draft",draftId));return b.toString();}

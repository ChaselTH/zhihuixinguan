...
final class CompletionRulePages extends PageLayout {
  ...
  CompletionRulePages returnTo(String url){returnUrl=ReturnNavigation.safe(url,"/");return this;}
  String settings(CompletionRules.Setting setting,boolean saved){
    ...
    StringBuilder b=new StringBuilder(backButton(returnUrl)).append("<div class=\"completion-settings\"><h1>填报必填设置</h1><p>按清单分别设置，对全部支行和所有期次生效，包括已导入的数据。修改规则只重新计算完成状态，不清空填写内容。</p>");
    if(saved)b.append("<p class=\"alert alert-success\">填报规则已保存，完成率已按新规则计算。</p>");
    b.append("<div class=\"completion-tabs\">");
    for(String type:java.util.List.of("multi","negative","cross"))b.append("<a class=\"btn ").append(type.equals(schema.id)?"btn-primary":"btn-light").append("\" href=\"").append(e(ReturnNavigation.link("/completion-rules?dataset="+type,returnUrl))).append("\">").append(e(DatasetSchema.get(type).label)).append("</a>");
    ...
    if(manager)b.append("<form method=\"post\" action=\"/completion-rules/save\" class=\"workflow-edit-form\">").append(hidden("csrf",currentSession.csrf)).append(hidden("return",returnUrl)).append(hidden("dataset",schema.id)).append(hidden("revision",""+setting.revision()));

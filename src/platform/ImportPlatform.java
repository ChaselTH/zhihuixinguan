...
public final class ImportPlatform {
  ...
  public record Job(String id,String dataset,String selectedMonth,String state,long revision,String createdAt,String expiresAt,int count,int examples,int repeated,String resultId) {}
  public record Summary(int fileCount,List<String> periods,Map<String,Integer> datasetCounts,Map<String,Integer> sheetCounts,int newCount,int formalDuplicates,int fillConflicts,int similarCandidates,int preserveUpdated,int overwriteUpdated) {
    public Summary { periods=List.copyOf(periods);datasetCounts=Collections.unmodifiableMap(new LinkedHashMap<>(datasetCounts));sheetCounts=Collections.unmodifiableMap(new LinkedHashMap<>(sheetCounts)); }
  }
  public record Preview(Job job,List<Item> items,Summary summary) {
    public Preview(Job job,List<Item> items){this(job,items,new Summary(0,List.of(),Map.of(),Map.of(),0,0,0,0,0,0));}
  ...
  @Deprecated public Job stage(ActorContext a,String dataset,List<SourceRow> input,int examples) {throw new IllegalArgumentException("导入暂存必须明确所属月份");}
  public Job stage(ActorContext a,String dataset,List<SourceRow> input,int examples,String selectedMonth) {String month=validatedMonth(selectedMonth);return call(a,()->stageInternal(a,dataset,withMonth(input,month),examples,month));}
  /** Stage one workbook containing all three supported data sheets as one atomic job. */
  @Deprecated public Job stageBundle(ActorContext a,List<SourceRow> input,int examples) {throw new IllegalArgumentException("导入暂存必须明确所属月份");}
  public Job stageBundle(ActorContext a,List<SourceRow> input,int examples,String selectedMonth) {String month=validatedMonth(selectedMonth);return call(a,()->stageInternal(a,"bundle",withMonth(input,month),examples,month));}
  private Job stageInternal(ActorContext a,String dataset,List<SourceRow> input,int examples,String selectedMonth)throws SQLException {
      ...
      selectedMonth=validatedMonth(selectedMonth);
      expire();
      ...
      for(var source:input){
        var r=source.record();DatasetSchema rowSchema=DatasetSchema.get(r.dataset());if(!bundle&&!dataset.equals(r.dataset()))throw new IllegalArgumentException("导入表种、机构或列数无效");if(!r.period().key().equals(selectedMonth))throw new IllegalArgumentException("暂存记录归档月份与本次选择不一致，请重新解析");validate(rowSchema,r);for(String value:r.values())characters+=value.length();if(characters>8_000_000)throw new IllegalArgumentException("本批单元格文字合计超过 800 万字，请分批上传");if(source.row()<1||source.sheet()==null||source.sheet().length()>200)throw new IllegalArgumentException("来源位置无效");
  ...
  public PlatformStore.ImportOutcome confirm(ActorContext a,String id,long revision,String mode,boolean allowClear,boolean allowSimilar) {
    return call(a,()->{
      ...
      editable(j,revision);
      if(j.selectedMonth().isBlank())throw new ConcurrentModificationException("旧导入任务没有冻结所属月份，请重新上传并选择月份；原任务未导入");
      rejectInFlightRows(id);
  ...
  private Job job(ActorContext a,String id)throws SQLException {
    try(var st=statement("SELECT * FROM import_jobs WHERE id=?",id);var rs=st.executeQuery()){
      ...
      Job j=readJob(rs);if(j.state().equals("PREVIEW")&&!clock.instant().isBefore(Instant.parse(j.expiresAt())))return new Job(j.id(),j.dataset(),j.selectedMonth(),"EXPIRED",j.revision(),j.createdAt(),j.expiresAt(),j.count(),j.examples(),j.repeated(),j.resultId());return j;
    }
  }
  ...
  private Summary summary(String id)throws SQLException {
    Set<String> files=new LinkedHashSet<>(),periods=new LinkedHashSet<>();Map<String,Integer> datasets=new LinkedHashMap<>(),sheets=new LinkedHashMap<>();int added=0,duplicates=0,conflicts=0,similar=0,preserveUpdated=0,overwriteUpdated=0;
    for(Item item:items(id,0,20000)){
      BusinessRecord incoming=item.source().record();files.add(incoming.filename());periods.add(incoming.period().key());datasets.merge(incoming.dataset(),1,Integer::sum);sheets.merge(item.source().sheet(),1,Integer::sum);similar+=item.similar();if(item.previous()==null)added++;else {duplicates++;DatasetSchema schema=DatasetSchema.get(incoming.dataset());for(int c=0;c<schema.width();c++)if(schema.editable(c)&&!schema.value(item.previous().values(),c).equals(schema.value(incoming.values(),c))){conflicts++;break;}
    ...
    return new Summary(files.size(),new ArrayList<>(periods),datasets,sheets,added,duplicates,conflicts,similar,preserveUpdated,overwriteUpdated);
  }
  private static Job readJob(ResultSet rs)throws SQLException{return new Job(rs.getString("id"),rs.getString("dataset"),Objects.toString(rs.getString("selected_month"),""),rs.getString("state"),rs.getLong("revision"),rs.getString("created_at"),rs.getString("expires_at"),rs.getInt("row_count"),rs.getInt("skipped_examples"),rs.getInt("repeated_rows"),Objects.toString(rs.getString("result_id"),""));}
  private void rejectInFlightRows(String id)throws SQLException {
    for(Item item:items(id,0,20000))if(item.previous()!=null) {
      String recordId=item.previous().id();String stage=scalar("SELECT stage FROM workflow_record_state WHERE record_id=?",recordId);
      String pending=scalar("SELECT COUNT(*) FROM pending_submission_records WHERE record_id=?",recordId);
      if(RowStage.BRANCH_REVIEW.name().equals(stage)||RowStage.DIVISION_REVIEW.name().equals(stage)||pending!=null&&Integer.parseInt(pending)>0)
        throw new ConcurrentModificationException("导入包含待审核记录，请先完成或退回待办，再重新预览；本批未导入");
    }
  }
  ...
  private static List<SourceRow> withMonth(List<SourceRow> rows,String month){var period=Period.parse(month,"");return rows.stream().map(source->{BusinessRecord r=source.record();return new SourceRow(new BusinessRecord(r.id(),r.version(),r.dataset(),period,r.organizationId(),r.values(),r.filename(),r.importedAt(),r.updatedAt(),r.legacyExtras(),r.workflowStage(),r.workflowReason()),source.sheet(),source.row());}).toList();}
  private static String validatedMonth(String value){if(value==null||!value.matches("20\\d{2}-(0[1-9]|1[0-2])"))throw new IllegalArgumentException("请选择有效所属月份（YYYY-MM）");try{YearMonth.parse(value);}catch(RuntimeException e){throw new IllegalArgumentException("请选择有效所属月份（YYYY-MM）");}return value;}

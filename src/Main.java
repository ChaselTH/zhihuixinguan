...
public final class Main extends HttpSupport {
  ...
  private void handle(HttpExchange x)throws IOException {
    ...
    try{
      ...
      if(method.equals("GET")){
        ...
        if(path.equals("/access/request")){var request=store.platform.access().application(session.actor,q.get("id"));sendHtml(x,200,access.application(request,store.platform.access().canDecide(session.actor,request),ReturnNavigation.safe(q.get("return"),"/access/requests")));return;}
        ...
        if(path.equals("/notifications/detail")){var notice=store.platform.access().notice(session.actor,q.get("id"));sendHtml(x,200,new NotificationPages(version,session).detail(notice,store.platform.access().noticeApplication(session.actor,notice.id()),q.get("return")));return;}
      ...
      if(method.equals("POST")){
        ...
        switch(path){
          ...
          case "/notifications/read-all" -> {store.platform.notifications().markAllRead(session.actor);session.unreadCount=store.platform.notifications().unreadCount(session.actor);redirect(x,ReturnNavigation.safe(f.get("return"),"/notifications"));return;}
          case "/notifications/open" -> {var notice=store.platform.access().notice(session.actor,f.get("id"));String application=store.platform.access().noticeApplication(session.actor,notice.id());String source=ReturnNavigation.safe(f.get("return"),"/notifications");store.platform.notifications().markRead(session.actor,notice.id());session.unreadCount=store.platform.notifications().unreadCount(session.actor);if(!application.isEmpty()){redirect(x,ReturnNavigation.link("/access/request?id="+url(application),source));return;}if(!notice.submissionId().isEmpty()){store.platform.workflow().submission(session.actor,notice.submissionId());redirect(x,ReturnNavigation.link("/workflow/submission?id="+url(notice.submissionId()),source));return;}redirect(x,ReturnNavigation.link("/notifications/detail?id="+url(notice.id()),source));return;}
          case "/access/decision" -> {String action=f.get("action");Role role="APPROVE".equals(action)?Role.valueOf(f.getOrDefault("role","")):null;var result=store.platform.access().decide(session.actor,f.get("id"),Long.parseLong(f.get("revision")),action,role,f.get("reason"),f.get("requestId"));if(result.created()!=null)sendHtml(x,200,identity.created(result.created()));else redirect(x,ReturnNavigation.link("/access/request?id="+url(result.application().id()),ReturnNavigation.safe(f.get("return"),"/access/requests")));return;}
  ...
  private void save(HttpExchange x,AuthService.Session session,Map<String,String> f)throws Exception {
    throw new SecurityException("此旧版直接修改入口已关闭，请由操作员在待处理清单发起填报");
  }

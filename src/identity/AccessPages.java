...
final class AccessPages extends PageLayout {
  ...
  String apply(String csrf,String scope) {
    ...
    StringBuilder b=new StringBuilder(header()).append("<div class=\"page-shell\"><section class=\"identity-card identity-narrow\"><h1>申请使用权限</h1><p>请选择机构类型。管理员核验身份后分配角色，并另行交付初始密码。</p><div class=\"access-tabs\"><a class=\"btn btn-light\" href=\"/access/apply?scope=division\">分行</a> <a class=\"btn btn-light\" href=\"/access/apply?scope=branch\">支行</a></div><h2>").append(branch?"支行人员申请":"分行人员申请").append("</h2><form method=\"post\" action=\"/access/apply\">").append(hidden("csrf",csrf));
    if(branch){b.append("<label>所属支行<select name=\"organization\">").append(option("","请选择支行",""));for(var o:Organizations.BRANCHES.entrySet())b.append(option(o.getKey(),o.getValue(),""));b.append("</select></label>");}else b.append(hidden("organization",Organizations.DIVISION));
    return page("申请权限",b.append("<label>统一认证号<input name=\"number\" maxlength=\"20\" autocomplete=\"off\"></label><label>姓名<input name=\"name\" maxlength=\"100\" autocomplete=\"off\"></label><button class=\"btn btn-primary\" type=\"submit\">提交申请</button> <a class=\"btn btn-light\" href=\"/login\">返回登录</a></form></section></div>").toString());
  }
  ...
  String applications(List<AccessPlatform.Application> rows,boolean pending,int offset) {
    StringBuilder b=new StringBuilder(header()).append("<div class=\"page-shell\"><h1>权限申请审批</h1><p>支行管理员可审批本支行操作员、复核员；需开通支行管理员时转交分行。分行人员仅由超级管理员审批。高层管理员可处理其管理范围内的申请。</p><p><a href=\"/access/requests\">待处理</a> · <a href=\"/access/requests?all=yes\">全部申请</a></p><div class=\"table-scroll\"><table class=\"data-table\"><thead><tr><th>申请时间</th><th>姓名</th><th>统一认证号</th><th>机构</th><th>状态</th><th>操作</th></tr></thead><tbody>");
    for(var r:rows)b.append("<tr><td>").append(e(time(r.createdAt()))).append("</td><td>").append(e(r.name())).append("</td><td>").append(e(r.number())).append("</td><td>").append(e(Organizations.label(r.organization()))).append("</td><td>").append(state(r.state())).append("</td><td><a href=\"/access/request?id=").append(u(r.id())).append("\">查看 / 审批</a></td></tr>");
    if(rows.isEmpty())b.append("<tr><td colspan=\"6\">暂无申请</td></tr>");

...
final class AccessPages extends PageLayout {
  ...
  String applications(List<AccessPlatform.Application> rows,boolean pending,int offset) {
    ...
    String source="/access/requests?all="+(pending?"no":"yes")+"&offset="+offset;
    for(var r:rows)b.append("<tr><td>").append(e(time(r.createdAt()))).append("</td><td>").append(e(r.name())).append("</td><td>").append(e(r.number())).append("</td><td>").append(e(Organizations.label(r.organization()))).append("</td><td>").append(r.requestedRole()==null?"历史申请":roleName(r.requestedRole())).append("</td><td>").append(state(r.state())).append("</td><td><a href=\"").append(e(ReturnNavigation.link("/access/request?id="+u(r.id()),source))).append("\">查看 / 审批</a></td></tr>");
  ...
  String application(AccessPlatform.Application r,boolean canDecide){return application(r,canDecide,"/access/requests");}
  String application(AccessPlatform.Application r,boolean canDecide,String source) {
    String back=ReturnNavigation.safe(source,"/access/requests");StringBuilder b=new StringBuilder(header()).append("<div class=\"page-shell\">").append(backButton(back)).append("<section class=\"identity-card identity-narrow\"><h1>权限申请详情</h1><p>姓名：").append(e(r.name())).append("</p><p>统一认证号：").append(e(r.number())).append("</p><p>机构：").append(e(Organizations.label(r.organization()))).append("</p><p>拟申请角色：").append(r.requestedRole()==null?"历史申请（需审批人明确分配）":roleName(r.requestedRole())).append("</p><p>状态：").append(state(r.state())).append("</p><p>处理说明：").append(e(r.reason())).append("</p>");
    ...
    return page("申请详情",b.append("</section></div>").toString());
  }
